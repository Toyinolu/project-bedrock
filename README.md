...
## Cost Controls — Teardown & Spin-up

EKS has **no free tier** (control plane bills ~$0.10/hr regardless of node count),
so when the environment is not being graded it is **fully destroyed** to reach
~$0/day. All infrastructure is reproducible from this repo on demand.

### Tear down (to ~$0/day)

```bash
# empty the versioned assets bucket first (or destroy will fail on it)
aws s3api list-object-versions --bucket bedrock-assets-alt-soe-025-3783 \
  --query '{Objects: Versions[].{Key:Key,VersionId:VersionId}}' --output json \
  | jq -c '. + {Quiet:true}' | aws s3api delete-objects \
  --bucket bedrock-assets-alt-soe-025-3783 --delete file:///dev/stdin

cd terraform && terraform destroy -auto-approve
```

The S3 **state backend** bucket (`project-bedrock-tfstate-...`) is NOT managed by
Terraform and survives — so spin-up reuses the same state key.

### Spin up for grading (~25–30 min)

```bash
# 1. rebuild the committed Helm chart dependencies
helm dependency build helm/chart/app/chart

# 2. apply all infrastructure
cd terraform
terraform init      # only needed on a fresh machine
terraform apply -auto-approve

# 3. fetch the (new) app URL + dev-view credentials to update the Google Doc
terraform output app_url
terraform output -raw bedrock_dev_view_access_key_id
terraform output -raw bedrock_dev_view_secret_access_key
terraform output -raw bedrock_dev_view_console_password
echo "Console: https://$(aws sts get-caller-identity --query Account --output text).signin.aws.amazon.com/console"
```

> A rebuild produces a **new ALB URL and new dev-view credentials**. Update the
> submitted Google Doc with these before the grader accesses the environment.
> (Allowed per grader confirmation: doc details may be updated after submission.)
